...
try:
    from monitoring.traffic_monitor import get_snapshot as get_traffic_snapshot
...
def _decide_with_route_classification(
    ...
    if route_label == ROUTE_MULTI_AGENT:
        selected_skills = []
        agent_skill_counts: dict[str, int] = {}
        agent_names: dict[str, str] = {}
        for skill in skills:
            acs = skill.get("acs") or skill.get("parent_agent") or {}
            aic = str(skill.get("aic") or skill.get("agent_id") or "").strip()
            if not aic:
                continue
            agent_name = str(skill.get("agent_name") or skill.get("agentName") or acs.get("name") or "").strip()
            selected_skills.append(
                {
                    "aic": aic,
                    "agent_name": agent_name,
                    "skillid": str(skill.get("skillid") or skill.get("skillId") or skill.get("id") or "").strip(),
                    "skill_name": str(skill.get("skill_name") or skill.get("skillName") or skill.get("name") or "").strip(),
                    "score": skill.get("score", skill.get("enhanced_weighted_score")),
                    "ranking": skill.get("ranking"),
                }
            )
            agent_skill_counts[aic] = agent_skill_counts.get(aic, 0) + 1
            if agent_name:
                agent_names[aic] = agent_name
        if selected_skills:
            best_agent_aic, best_agent_skill_count = max(agent_skill_counts.items(), key=lambda item: item[1])
            evidence.update(
                {
                    "relevant_skill_count": len(selected_skills),
                    "distinct_agent_count": len(agent_skill_counts),
                    "best_agent": {
                        "aic": best_agent_aic,
                        "name": agent_names.get(best_agent_aic, best_agent_aic),
                        "skill_count": best_agent_skill_count,
                        "coverage": round(best_agent_skill_count / len(selected_skills), 4),
                    },
                    "agent_skill_counts": dict(
                        sorted(agent_skill_counts.items(), key=lambda item: item[1], reverse=True)
                    ),
...
def _extract_agent_output(payload: Any) -> str:
    ...
    def _text_from_task_result(obj: Any) -> str:
        ...
        if isinstance(status, Mapping):
            text = _text_from_items(status.get("dataItems") or status.get("data_items"))
            if text:
                return text
            ...
            for product in products:
                if isinstance(product, Mapping):
                    text = (
                        ...
                        else _text_from_items(product.get("dataItems") or product.get("data_items"))
                    )
                    if text:
                        return text
        return _text_from_items(obj.get("dataItems") or obj.get("data_items"))
            ...
            for product in products:
                ...
                for item in data_items:
                    if isinstance(item, Mapping) and item.get("text"):
                        return str(item["text"])
        data_items = result.get("dataItems") or result.get("data_items") or []
    ...
    return {
        ...
        "params": {
            "command": {
                ...
                "command": "start",
                "dataItems": _command_text_data_items(task),
            },
            "text": task,
            "task": task,
...
def _float_from_payload(payload: Mapping[str, Any], keys: Iterable[str], default: float) -> float:
    for key in keys:
...
def _agent_timeout_from_payload(payload: Mapping[str, Any], *, default: float = 30.0, max_default: float = 30.0) -> float:
    explicit_keys = (
        "agent_timeout",
        "agentTimeout",
        "mode_agent_timeout",
    ...
    if any(key in payload and payload.get(key) not in (None, "") for key in explicit_keys):
        return max(1.0, _float_from_payload(payload, explicit_keys, default))
    generic_timeout = _float_from_payload(payload, ("timeout",), default)
    return max(1.0, min(generic_timeout, max_default))


def _execution_timeout_from_payload(payload: Mapping[str, Any], *, default: float = 150.0, max_default: float = 150.0) -> float:
    explicit_keys = (
        "execution_timeout",
        "executionTimeout",
...
def execute_plan_single_agent(task: str, plan: Mapping[str, Any], *, payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    started = time.perf_counter()
    timeout = _agent_timeout_from_payload(payload, default=30.0, max_default=30.0)
    ...
    try:
        raw_response = _post_json(endpoint, request_payload, timeout)
        output_text = _extract_agent_output(raw_response)
        failed = output_text.startswith("Agent RPC error:")
        status = "failed" if failed else "completed"
        error = output_text if failed else ""
    except Exception as exc:
        raw_response = {}
        output_text = ""
        status = "failed"
...
def execute_plan_http_agents(task: str, plan: Mapping[str, Any], *, payload: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    timeout = _agent_timeout_from_payload(payload, default=30.0, max_default=30.0)
    execution_deadline = time.perf_counter() + _execution_timeout_from_payload(payload, default=150.0, max_default=150.0)
    events: list[dict[str, Any]] = [
        {
            "timestamp": _utc_now(),
            "state": "running",
    ...
    while pending:
        ...
        if not ready:
            ready = pending[:1]
        if time.perf_counter() >= execution_deadline:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            remaining = max(0.1, execution_deadline - time.perf_counter())
            ...
            for future in pending_futures:
                ...
                run["duration_ms"] = round(max(0, timeout) * 1000)
                run_by_package[package_id] = run
                completed.add(package_id)
                events.append(
                    {
    ...
    final_result = "\n\n".join(
        f"## {run.get('agent_name') or run['package_id']}\n{run.get('output_text') or ''}"
...
def _build_orchestrator_execute_response(task: str, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if plan.get("status") == "dispatch_blocked":
        ...
        if execution_transport in {"group", "group_chat", "rabbitmq", "rabbitmq_group_chat"}:
