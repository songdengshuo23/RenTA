...
class OCSPService:
    ...
    @staticmethod
    def _serial_lookup_candidates(serial_number_int: int) -> List[str]:
        """为 OCSP 请求中的序列号生成兼容候选值。

        历史数据中既有十进制字符串，也有真实证书序列号常见的十六进制字符串，
        因此这里按顺序做兼容查询，优先匹配精确的十进制形式。
        """
        decimal_serial = str(serial_number_int)
        hex_serial_upper = format(serial_number_int, "X")
        hex_serial_lower = hex_serial_upper.lower()

        candidates: List[str] = []
        for candidate in (decimal_serial, hex_serial_upper, hex_serial_lower):
            if candidate not in candidates:
                candidates.append(candidate)
        return candidates

    def _get_certificate_for_ocsp_serial(
        self, serial_number_int: int
    ) -> Optional[Certificate]:
        """按 OCSP 请求序列号查找证书，兼容十进制和十六进制存储格式。"""
        for candidate in self._serial_lookup_candidates(serial_number_int):
            certificate = self.db.exec(
                select(Certificate).where(Certificate.serial_number == candidate)
            ).first()
            if certificate:
                return certificate
        return None
    ...
    def process_ocsp_request(
        ...
        try:
            ...
            # 查询证书状态
            certificate = self._get_certificate_for_ocsp_serial(serial_number_int)
