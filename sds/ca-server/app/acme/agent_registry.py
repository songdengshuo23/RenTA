...
class AgentRegistryClient:
    ...
    async def verify_agent_ownership(
        ...
        Registry currently has no dedicated ACME-account ownership API. In real mode
        we therefore fail closed for inactive/missing AICs, enforce any explicit
        externalAccountBinding AIC claim, and leave final proof of control to the
        protected HTTP-01 challenge.
        """
        # Mock 模式
        if self.is_mock_enabled:
            print(
        ...
        try:
            agent_info = await self.validate_aic_and_get_info(aic)
            if not agent_info:
                print(f"Ownership check failed: invalid or inactive AIC {aic}")
                return False

            binding = account_info.get("external_account_binding") or {}
            bound_aics = self._extract_bound_aics(binding)
            if bound_aics:
                if aic in bound_aics:
                    print(f"Agent ownership verified by externalAccountBinding: {aic}")
                    return True
                print(
                    f"Ownership check failed: AIC {aic} not in account binding {bound_aics}"
                )
                return False

            print(
                f"Ownership pre-check passed for active AIC {aic}; "
                "HTTP-01 challenge will prove endpoint control"
            )
            return True
    ...
    def _extract_bound_aics(self, binding: Dict[str, Any]) -> set[str]:
        """Extract AIC claims from externalAccountBinding-like payloads."""
