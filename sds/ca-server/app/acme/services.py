...
from app.common import (
    Certificate,
    CertificateType,
    CertificateStatus as CommonCertificateStatus,
    RevocationReason as CommonRevocationReason,
)
...
class CertificateService:
    ...
    def revoke_certificate(
        ...
        self.sync_common_certificate_revocation(certificate)

        self.session.add(certificate)
        self.session.commit()

    def sync_common_certificate_revocation(
        self, certificate: AcmeCertificate
    ) -> None:
        """Keep the common certificate record aligned with ACME revocation state."""
        statement = select(Certificate).where(
            Certificate.serial_number == certificate.serial_number
        )
        common_cert = self.session.exec(statement).first()
        if not common_cert:
            return

        common_cert.status = CommonCertificateStatus.REVOKED
        common_cert.revoked_at = certificate.revoked_at or beijing_now()
        common_cert.revocation_reason = (
            CommonRevocationReason(certificate.revocation_reason.value)
            if certificate.revocation_reason
            else None
        )
        common_cert.updated_at = beijing_now()
    ...
    def _create_certificate(self, cert_data: CertificateCreate) -> AcmeCertificate:
        ...
        if cert_data.aic:
            ...
            common_cert = Certificate(
                ...
                status=CommonCertificateStatus.VALID,
                certificate_pem=cert_data.certificate_pem,
