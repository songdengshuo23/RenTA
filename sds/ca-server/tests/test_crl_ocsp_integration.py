...
class TestCRLOCSPBasicIntegration:
    ...
    def test_certificate_lifecycle_integration(
        self, client: TestClient, test_certificates, admin_auth_headers
    ):
        """测试证书生命周期的完整集成"""
        # 找到一个有效证书用于测试
        valid_cert = None
...
class TestCRLOCSPErrorHandling:
    ...
    def test_invalid_certificate_id_revocation(self, client: TestClient, admin_auth_headers):
        """测试使用无效证书ID进行吊销"""
...
class TestCRLOCSPDataConsistency:
    ...
    def test_database_transaction_consistency(
        self, client: TestClient, test_certificates, db_session, admin_auth_headers
    ):
        """测试数据库事务的一致性"""
        valid_cert = None
        for cert in test_certificates:
