...
def client(admin_auth_headers):
    """创建默认携带 CA Admin Token 的测试客户端。

    当前证书管理接口属于 /admin 管理面，生产代码会要求 Bearer Token。
    这里补齐认证头，避免测试因为缺少安全上下文而全部返回 401。
    """
...
class TestCertificateService:
    ...
    def test_certificate_expiry_calculation(self, client):
        ...
        if expires_at.tzinfo is None:
            expires_at = expires_at.replace(tzinfo=before_creation.tzinfo)
