...
class TestOCSPService:
    ...
    def test_process_ocsp_request_matches_hex_serial_storage(
        self, ocsp_service, ocsp_crypto_setup, monkeypatch, db_session
    ):
        """标准 OCSP 请求里的整数序列号应能命中十六进制存储的证书记录。"""

        certificate = db_session.get(Certificate, ocsp_crypto_setup.certificate_id)
        certificate.serial_number = format(int(ocsp_crypto_setup.serial_number), "X")
        db_session.add(certificate)
        db_session.commit()
        ...
        response_der, _ = ocsp_service.process_ocsp_request(ocsp_crypto_setup.request_der)
...
class TestOCSPIntegration:
    ...
    def test_certificate_revocation_updates_ocsp(
        self, client: TestClient, valid_certificate, admin_auth_headers
    ):
        """测试证书吊销后OCSP状态更新"""
        # 初始状态应该是good
