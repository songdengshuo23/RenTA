...
def app():
    """创建用于测试的FastAPI应用实例"""
    api_module.settings.DSP_SERVICE_TOKEN = "test-dsp-token"
    app = FastAPI()
...
def client(app):
    """创建用于测试的HTTP客户端"""
    return TestClient(app, headers={"Authorization": "Bearer test-dsp-token"})
...
@pytest.fixture
def bad_auth_client(app):
    return TestClient(app, headers={"Authorization": "Bearer wrong"})
