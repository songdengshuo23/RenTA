...
if [ -x "$SCRIPT_DIR/.venv/bin/python" ]; then
  ...
  VENV_DIR="$SCRIPT_DIR/venv"
  PYTHON_BIN="$VENV_DIR/bin/python"
elif [ -d "$SCRIPT_DIR/.venv" ] && command -v python3 >/dev/null 2>&1; then
  VENV_DIR="$SCRIPT_DIR/.venv"
...
PYTHON_EXTRA_PATHS=("$SCRIPT_DIR")
if [ -d "$SCRIPT_DIR/.py312deps" ]; then
  PYTHON_EXTRA_PATHS+=("$SCRIPT_DIR/.py312deps")
fi
if [ -d "$VENV_DIR/lib/python3.13/site-packages" ]; then
  PYTHON_EXTRA_PATHS+=("$VENV_DIR/lib/python3.13/site-packages")
fi
REGISTRY_PYTHONPATH="$(IFS=:; echo "${PYTHON_EXTRA_PATHS[*]}")"
LOCAL_DATABASE_URL="${LOCAL_DATABASE_URL:-postgresql+asyncpg://sds:sds123@127.0.0.1:5432/registry_db}"

# 从配置读取默认端口
get_configured_port() {
  DATABASE_URL="${DATABASE_URL:-$LOCAL_DATABASE_URL}" PYTHONPATH="$REGISTRY_PYTHONPATH" "$PYTHON_BIN" -c \
...
do_start() {
  ...
  # 启动服务
  DATABASE_URL="${DATABASE_URL:-$LOCAL_DATABASE_URL}" PYTHONPATH="$REGISTRY_PYTHONPATH" nohup "$PYTHON_BIN" -u "$SCRIPT_DIR/$ENTRY_POINT" \
      >>"$LOG_FILE" 2>&1 &
  echo $! >"$PID_FILE"
