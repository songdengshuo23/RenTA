...
class Settings(BaseSettings):
    ...
    CA_SERVER_SERVICE_TOKEN: str = os.getenv("CA_SERVER_SERVICE_TOKEN", "")
    PASSPORT_CERT_VALIDITY_DAYS: int = int(
        os.getenv("PASSPORT_CERT_VALIDITY_DAYS", "49")
    )
    PASSPORT_CERT_RENEWAL_WINDOW_DAYS: int = int(
        os.getenv("PASSPORT_CERT_RENEWAL_WINDOW_DAYS", "14")
    )
    PASSPORT_DRAFT_REVIEW_AFTER_DAYS: int = int(
        os.getenv("PASSPORT_DRAFT_REVIEW_AFTER_DAYS", "7")
    )
    ...
    REGISTRY_SERVICE_TOKEN: str = os.getenv("REGISTRY_SERVICE_TOKEN", "")

    # DSP (Data Synchronization Protocol) settings
    DSP_BASE_PATH: str = os.getenv("DSP_BASE_PATH", "/acps-dsp-v2")
    DSP_SERVICE_TOKEN: str = os.getenv("DSP_SERVICE_TOKEN", "")
