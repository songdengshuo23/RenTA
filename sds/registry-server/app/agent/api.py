...
from app.events.hub import publish_agent_event
import json
import os
import logging
...
async def client_create_new_agent(
    ...
    publish_agent_event(
        ...
        level="success",
        title="Agent 已创建",
        message=f"{agent.name} {agent.version} 已保存为草稿，可以提交审核。",
        agent=agent,
...
@router_client.post("/{agent_id}/submit", response_model=AgentSubmitResponse)
async def client_submit_agent(
    ...
    """
    Submit an Agent for review and return the persisted Supervisor review.
    """
    agent = submit_agent_for_approval(db, agent_id, current_user.id)
    agent = get_agent(db, agent_id, with_users=False)
    publish_agent_event(
        event_type="agent.submitted",
        level="info",
        title="Agent 已提交审核",
        message=f"{agent.name} 已进入 Registry 审核流程。",
        agent=agent,
        user_id=str(current_user.id),
    )
    supervisor_review = run_review_and_persist_passport(db, agent)
    reviewed_agent = get_agent(db, agent_id, with_users=False)
    decision = str(supervisor_review.get("decision") or "")
    review_level = "success" if decision == "APPROVE" else "warning" if decision == "MANUAL_REVIEW" else "error"
    review_title = "审核通过" if decision == "APPROVE" else "需要人工审核" if decision == "MANUAL_REVIEW" else "审核未通过"
    review_summary = {
        "reviewId": supervisor_review.get("reviewId"),
        "decision": supervisor_review.get("decision"),
        "riskLevel": supervisor_review.get("riskLevel"),
        "permissionTier": supervisor_review.get("permissionTier"),
        "conclusion": supervisor_review.get("conclusion"),
        "humanReviewRequired": supervisor_review.get("humanReviewRequired"),
    }
    publish_agent_event(
        event_type="agent.review.completed",
        level=review_level,
        title=review_title,
        message=f"{reviewed_agent.name} 审核结果：{decision or 'UNKNOWN'}，风险等级 {supervisor_review.get('riskLevel') or '-'}。",
        agent=reviewed_agent,
        review=review_summary,
        user_id=str(current_user.id),
    )
    latest_passport = get_latest_agent_passport(db, agent_id)
    if latest_passport:
        passport_level = "success" if latest_passport.status == "VALID" else "warning"
        publish_agent_event(
            event_type="agent.passport.issued",
            level=passport_level,
            title="Passport 已生成",
            message=f"{reviewed_agent.name} Passport 状态：{latest_passport.status}，权限层级 {latest_passport.permission_tier}。",
            agent=reviewed_agent,
            passport=latest_passport,
            user_id=str(current_user.id),
        )
    response = create_agent_response(agent)
    return AgentSubmitResponse(
        **response.model_dump(), supervisor_review=supervisor_review
    )


@router_client.get(
    "/{agent_id}/passport/latest",
    response_model=AgentPassportResponse,
)
async def client_get_latest_agent_passport(
    agent_id: uuid.UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(check_user_role([RoleType.CLIENT])),
):
    agent = get_agent(db, agent_id, with_users=False, raise_exception=True)
    if agent.created_by_id != current_user.id:
        raise AgentException(
            status_code=status.HTTP_403_FORBIDDEN,
            error_name=AgentError.ACCESS_DENIED_NOT_OWNER,
            error_msg="Access denied: you can only view passports for agents that you created",
            input_params={
                "agent_id": str(agent_id),
                "request_user_id": str(current_user.id),
            },
    ...
    passport = get_latest_agent_passport(db, agent_id)
    if not passport:
        raise AgentException(
            status_code=status.HTTP_404_NOT_FOUND,
            error_name=AgentError.AGENT_PASSPORT_NOT_FOUND,
            error_msg="Agent Passport not found",
            input_params={"agent_id": str(agent_id)},
        )
    return create_agent_passport_response(passport)
...
async def client_get_agent_passport(
    agent_id: uuid.UUID,
    passport_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(check_user_role([RoleType.CLIENT])),
    ...
    passport = get_agent_passport_by_passport_id(db, agent_id, passport_id)
    if not passport:
        raise AgentException(
            status_code=status.HTTP_404_NOT_FOUND,
            error_name=AgentError.AGENT_PASSPORT_NOT_FOUND,
            error_msg="Agent Passport not found",
            input_params={"agent_id": str(agent_id), "passport_id": passport_id},
        )
    return create_agent_passport_response(passport)


@router_client.get(
    "/{agent_id}/supervisor-review/latest",
    response_model=AgentSupervisorReviewResponse,
)
async def client_get_latest_supervisor_review(
    agent_id: uuid.UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(check_user_role([RoleType.CLIENT])),
):
    ...
    if agent.created_by_id != current_user.id:
        raise AgentException(
            status_code=status.HTTP_403_FORBIDDEN,
            error_name=AgentError.ACCESS_DENIED_NOT_OWNER,
            error_msg="Access denied: you can only view reviews for agents that you created",
            input_params={
                "agent_id": str(agent_id),
                "request_user_id": str(current_user.id),
            },
    ...
    review = get_latest_supervisor_review(db, agent_id)
    if not review:
        raise AgentException(
            status_code=status.HTTP_404_NOT_FOUND,
            error_name=AgentError.SUPERVISOR_REVIEW_NOT_FOUND,
            error_msg="Supervisor review not found",
            input_params={"agent_id": str(agent_id)},
        )
    return create_supervisor_review_response(review)


@router_client.get(
    "/{agent_id}/supervisor-review/{review_id}",
    response_model=AgentSupervisorReviewResponse,
)
async def client_get_supervisor_review(
    agent_id: uuid.UUID,
    review_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(check_user_role([RoleType.CLIENT])),
    ...
    review = get_supervisor_review_by_review_id(db, agent_id, review_id)
    if not review:
        raise AgentException(
            status_code=status.HTTP_404_NOT_FOUND,
            error_name=AgentError.SUPERVISOR_REVIEW_NOT_FOUND,
            error_msg="Supervisor review not found",
            input_params={"agent_id": str(agent_id), "review_id": review_id},
        )
    return create_supervisor_review_response(review)
