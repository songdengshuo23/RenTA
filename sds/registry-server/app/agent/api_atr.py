...
from app.agent.service import (
    ...
    get_agent_by_aic,
    get_latest_agent_passport_by_aic,
...
@router.get("/passports/discovery")
async def list_discovery_passports(
    limit: int = 100,
    ...
    """
    Return Passport summaries that Discovery can index.

    Only VALID and APPROVE passports for active Agents are returned, so rejected,
    suspended, disabled or deleted Agents do not enter the normal discovery feed.
    """
    bounded_limit = max(1, min(limit, 500))
    items = list_discovery_passport_summaries(
        db, limit=bounded_limit, requester_user_id=requesterUserId
    )
    return JSONResponse(
        content={
            "status": "ok",
...
@router.post("/passports/runtime-review/schedule")
async def schedule_passport_runtime_reviews(
    limit: int = 100,
    syncCertificates: bool = False,
    _auth: None = Depends(require_registry_service_token),
    db: Session = Depends(get_db),
):
    """
    Run the Passport runtime-review scheduler once.

    This internal endpoint is meant for cron/orchestrator jobs. It scans Passport
    expiry, review_after, ACS drift, endpoint drift and certificate renewal due
    conditions. When syncCertificates=true, it also asks CA Server to renew due
    certificates and writes the result back into the Passport.
    """
    result = run_passport_runtime_review_scheduler(
        db,
        limit=limit,
        sync_certificates=syncCertificates,
    )
    return JSONResponse(content={"status": "ok", "result": result})


@router.get("/passports/{agent_aic}/dispatch")
async def get_passport_dispatch_eligibility(
    agent_aic: str,
    requesterUserId: Optional[str] = None,
    _auth: None = Depends(require_registry_service_token),
    db: Session = Depends(get_db),
    ...
    """
    Return the latest Passport with Orchestrator dispatch eligibility.

    Orchestrator reads this before calling an Agent and can enforce the returned
    permission tier, task classes and dispatch reasons without re-running review.
    """
    ...
    agent, passport = get_latest_agent_passport_by_aic(db, agent_aic)
    if not agent:
        raise AtrException(
    ...
    if not passport:
        raise AtrException(
            code=AtrErrorCode.AGENT_PASSPORT_NOT_FOUND,
    ...
    return JSONResponse(
        content={
            "status": "ok",
            "result": build_passport_dispatch_view(
                agent, passport, requester_user_id=requesterUserId
            ),
        }
    )
...
async def register_entity_endpoint(
    ...
    根据 ATR-Registry-Server.md 规范：
    - 请求方通过 mTLS 认证（使用本体证书）；无 mTLS 网关时使用服务令牌兜底
