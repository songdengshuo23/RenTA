...
class AgentSubmitResponse(AgentResponse):
    """Agent submission response with the persisted Supervisor review."""

    supervisor_review: Optional[Dict[str, Any]] = None


class AgentSupervisorReviewResponse(BaseModel):
    id: uuid.UUID
    review_id: str
    agent_id: uuid.UUID
    agent_acs_hash: Optional[str] = None
    agent_acs_version: int
    review_mode: str
    decision: str
    risk_level: str
    permission_tier: str
    ...
    passport_draft: Optional[Dict[str, Any]] = None
    ...
    @classmethod
    def convert_review_datetime_to_beijing(cls, v):
        if v is not None:
            beijing_time = utc_to_beijing(v)
            if beijing_time.tzinfo is not None:
                return beijing_time
...
class AgentPassportResponse(BaseModel):
    id: uuid.UUID
    passport_id: str
    agent_id: uuid.UUID
    review_id: str
    ...
    @classmethod
    def convert_passport_datetime_to_beijing(cls, v):
        if v is not None:
            beijing_time = utc_to_beijing(v)
            if beijing_time.tzinfo is not None:
                return beijing_time
