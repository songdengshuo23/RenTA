...
# ===========================================================================
PROJECT_NAME="ca-challenge"                       # 项目名称（仅用于显示）
SVC_NAME="challenge"                               # 服务名（PID/日志文件前缀）
ENTRY_POINT="main.py"                              # 启动入口脚本
PORT_CONFIG_EXPR="settings.UVICORN_PORT"          # Python 表达式：从配置读取端口
DEFAULT_PORT="8004"                               # 配置读取失败时的回退端口
...
get_configured_port() {
  PYTHONPATH="$SCRIPT_DIR" "$PYTHON_BIN" -c \
    "from acps_ca_challenge.config import settings; print($PORT_CONFIG_EXPR)" \
