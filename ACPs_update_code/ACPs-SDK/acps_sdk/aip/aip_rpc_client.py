...
from ..monitoring.traffic_monitor import count_tokens, serialize_payload, record as _traffic_record
...
class AipRpcClient:
    ...
    async def _send_request(self, command: TaskCommand) -> RpcResponse:
        ...
        try:
            payload_text = serialize_payload(rpc_request)
            tokens = count_tokens(payload_text)
            url_parts = self.partner_url.rstrip("/").split("/")
            target_agent = url_parts[-2] if len(url_parts) >= 2 and url_parts[-1] == "rpc" else self.partner_url
            _traffic_record(source=self.leader_id, target=target_agent, tokens=tokens)
        except Exception:
            pass
