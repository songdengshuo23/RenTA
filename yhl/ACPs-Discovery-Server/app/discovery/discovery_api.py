...
async def publish_runtime_event(
    *,
    ...
    payload = {
        ...
        "title": title,
        "message": message,
        "extra": {"service": "discovery-server", **(extra or {})},
    }
    try:
        ...
        logger.debug("failed to publish discovery runtime event: %s", exc)
...
async def discover_agents_post(request: DiscoveryRequest) -> DiscoveryResponse:
    ...
    if request.type == "filtered":
        ...
        try:
            ...
            await publish_runtime_event(
                event_type="discovery.query.completed",
                level="success",
                title="Discovery 过滤查询完成",
                message=f"Discovery filtered 查询完成，返回 {len(agent_response)} 个 Agent。",
                extra={
                    "stage": "discover-filtered",
                    "path": "/acps-adp-v2/discover",
                    "query": request.query,
                    "limit": request.limit,
                    "type": request.type,
                    "agentCount": len(agent_response),
                    "durationMs": int(durationMs),
                },
            )
            ...
            await publish_runtime_event(
                event_type="discovery.query.failed",
                level="error",
                title="Discovery 过滤查询失败",
                message=f"Discovery filtered 查询失败：{e}",
                extra={
                    "stage": "discover-filtered",
                    "path": "/acps-adp-v2/discover",
                    "query": request.query,
    ...
    if config.forwarder_server_enabled and get_forwarder_health_status():
        ...
        if f_response is not None:
            ...
            await publish_runtime_event(
                event_type="discovery.query.completed",
                level="success",
                title="Discovery 转发查询完成",
                message="Discovery 查询已由 forwarder 返回结果。",
                extra={
                    "stage": "discover-forwarder",
                    "path": "/acps-adp-v2/discover",
                    "query": request.query,
                    "limit": request.limit,
                    "type": request.type,
    ...
    try:
        ...
        await publish_runtime_event(
            event_type="discovery.query.completed",
            level="success",
            title="Discovery 本地查询完成",
            message=f"Discovery 本地查询完成，返回 {len(agent_response)} 个 Agent。",
            extra={
                "stage": "discover-local",
                "path": "/acps-adp-v2/discover",
                "query": request.query,
                "limit": request.limit,
                "type": request.type,
                "usedForwarder": used_forwarder,
                "forwarderSuccess": forwarder_success,
                "agentCount": len(agent_response),
                "durationMs": int(durationMs),
            },
        )
        ...
        await publish_runtime_event(
            event_type="discovery.query.failed",
            level="error",
            title="Discovery 查询失败",
            message=f"Discovery 查询失败：{type(e).__name__}: {e}",
            extra={
                "stage": "discover-local",
                "path": "/acps-adp-v2/discover",
                "query": request.query,
...
async def discover_agents_v1(request: DiscoveryRequestV1) -> DiscoveryResponseV1:
    ...
    try:
        ...
        await publish_runtime_event(
            event_type="discovery.query.completed",
            level="success",
            title="Discovery V1 查询完成",
            message=f"Discovery V1 查询完成，返回 {len(v1_agents)} 个 Agent。",
            extra={
                "stage": "discover-v1",
                "path": "/acps-adp-v2/discover/v1",
                "query": request.query,
                "limit": request.limit,
                "agentCount": len(v1_agents),
                "durationMs": round((time.perf_counter() - started) * 1000),
            },
        )
        ...
        await publish_runtime_event(
            event_type="discovery.query.failed",
            level="error",
            title="Discovery V1 查询失败",
            message=f"Discovery V1 查询失败：{type(e).__name__}: {e}",
            extra={
                "stage": "discover-v1",
                "path": "/acps-adp-v2/discover/v1",
                "query": request.query,
