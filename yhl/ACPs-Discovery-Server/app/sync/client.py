...
class DRCClient:
    ...
    def _auth_headers(self, accept: str) -> dict:
        ...
        token = getattr(settings, "DRC_SERVICE_TOKEN", "")
        if token:
    ...
    async def get_registry_info(self) -> Optional[RegistryInfo]:
        ...
        try:
            ...
            response = await client.get(url, headers=self._auth_headers("application/json"))
    ...
    async def create_snapshot(
        ...
        try:
            ...
            response = await client.get(
                url, params=params, headers=self._auth_headers("application/x-ndjson")
            )
            ...
            for chunk_index in range(1, snapshot_info.chunk_total):
                ...
                chunk_response = await client.get(
                    url, params=chunk_params, headers=self._auth_headers("application/x-ndjson")
                )
