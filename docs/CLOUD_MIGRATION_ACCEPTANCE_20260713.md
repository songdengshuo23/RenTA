...
平台内部、网关、PC 浏览器、协议同步和 RabbitMQ mTLS 均已通过验收。2026-07-14 已增加 Nginx 80 端口入口并完成公网直连验证；外部设备可直接访问 `http://120.27.205.185/`，原 `:8888` 入口继续兼容。
...
| Public HTTP | `nginx.service` | `0.0.0.0:80` | 公网 IP 默认入口，透明代理到统一网关 8888 |
...
主机 `firewalld` 已放行 HTTP 80 和兼容网关 8888，公网侧已实际验证可达：

| 协议 | 端口 | 建议来源 | 用途 |
| --- | --- | --- | --- |
| TCP | `80` | `0.0.0.0/0` | RenTA 默认 Web 平台与统一 HTTP API |
...
不要开放 `8001`、`8003`、`8004`、`8005`、`8098`、`8099`、`9007`、`9008`、`15672`、`5432` 或 `6379`。默认入口为 `http://120.27.205.185/`，兼容入口为 `http://120.27.205.185:8888/`。
